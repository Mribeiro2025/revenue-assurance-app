  ...
  for caminho_arq, nome_fonte in fontes:
    ...
    for r in tqdm(
      ...
      if (
        ...
        ponto_venda_final = (
            curr_ponto_venda
            or d_oficial.get("PONTO DE VENDA")
            or "Não Mapeado"
        )
        iata_final = curr_iata or d_oficial.get("CÓDIGO IATA") or "DIRETO"

        iata_clean = clean_iata(iata_final)
        m_iata = iata_dict.get(iata_clean, {})

        gerente_resp = (
            d_oficial.get("GERENTES")
            or d_oficial.get("ÁREA RESPONSÁVEL")
            or m_iata.get("Gerente_Responsavel", "Não Mapeado")
        )
        ...
        if b_match:
          ...
          aba_destino = (
              ...
              if status_divergencia == "Valores Corretos"
              else "98_OK_Divergencia_Operacao"
          )
        else:
