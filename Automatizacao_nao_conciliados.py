...
def carregar_tratativas_e_logs_anteriores(out_file):
    ...
    try:
        ...
        df_log_antigo = (
            pd.read_excel(xls, sheet_name="00_Log_Auditoria")
...
def executar_auditoria():
    ...
    dict_historico, df_log_acumulado = carregar_tratativas_e_logs_anteriores(
        out_file_model
    )
    ...
    if caminho_lemon:
        ...
        for idx, r in tqdm(
            df_lemon.iterrows(),
            total=len(df_lemon),
            desc="Indexando Lemontech",
            unit="reg",
        ):
            cc_desc = (
                f"{clean_str_strict(r.get('Centro de Custo'))} -"
                f" {clean_str_strict(r.get('Descrição Centro de Custo'))}"
            ).strip(" -")
            info_lemon = {
                ...
                "Lemon_Emissor_Reserva": (
                    clean_str_strict(r.get("Emissor Reserva")) or "-"
                ),
                "Lemon_Centro_Custo": cc_desc if cc_desc else "-",
                "Lemon_Forma_Pagto": (
                    clean_str_strict(r.get("Forma de Pagamento")) or "-"
                ),
                "Lemon_Autorizacao_Cartao": (
                    clean_str_strict(r.get("Autorização do Cartão")) or "-"
                ),
    ...
    print("\n[3/5] Indexando ERP Benner (Multicolunas & Rloc CIA)...")
    ...
    if not df_benner.empty:
        for idx, r in tqdm(
            ...
            info = {
                ...
                "Benner_Fornecedor": clean_str_strict(
                    r.get("Apelido Fornecedor") or r.get("Fornecedor")
                )
                or "-",
                ...
                + clean_num(r.get("Taxa BR")),
                "Benner_Receita": clean_num(r.get("Comissão"))
                + clean_num(r.get("Taxa DU"))
                + clean_num(r.get("Incentivo"))
                + clean_num(r.get("Fee"))
                + clean_num(r.get("RAV")),
                "Benner_Cliente": clean_str_strict(
                    r.get("Cliente") or r.get("Apelido Cliente")
                )
                or "-",
                "Benner_Emissor": clean_str_strict(
                    r.get("Emissor") or r.get("Agente Criação")
                )
                or "-",
                "Benner_Passageiro": clean_str_strict(r.get("Passageiro")) or "-",
                "Benner_Sistema_Reserva": clean_str_strict(r.get("Sistema Reserva"))
                or "-",
            }
    ...
    if caminho_sabre:
        ...
        for line in lines:
            if (
                "Total Amount" in line
                or "Total Commission" in line
                or "PCC,DATE,AL CODE" in line
            ):
                continue
    ...
    for lista_nomes, nome_fonte in fontes:
        ...
        for r in tqdm(
            range(len(df_raw)), desc=f"Auditando {nome_fonte}", unit="linha"
        ):
            ...
            col6 = clean_str_strict(df_raw.iloc[r, 6])
            ...
            if (
                col3 != ""
                and col3 not in ["BILHETE\\RLOC", "BILHETE", "RLOC", "[]"]
                and col1 != "CIA"
            ):
                if col3.upper() in [
                    "BILHETE\\RLOC",
                    "BILHETE",
                    "RLOC",
                ] or col5.upper() in ["EMISSÃO", "EMISSAO"]:
                    continue
                ...
                doc_val = (
                    clean_str_strict(df_raw.iloc[r, 22])
                    if df_raw.shape[1] > 22
                    else ""
                )

                hist_data = dict_historico.get(bilhete_chave, {})

                ponto_venda_final = (
                    curr_ponto_venda
                    or hist_data.get("Ponto de venda")
                    or "Não Mapeado"
                )
                ...
                area_resp_oficial = gerente_resp
                ...
                if hist_data.get("Obs. Operação"):
                    obs_op = hist_data["Obs. Operação"]
                ...
                if e_suporte_backoffice:
                    ...
                    "JAIME SCHNAIDER" in area_resp_upper or "UNIQUE" in area_resp_upper
                ):
                    setor_final = "Unique"
                elif (
                    "CENTRAL DE EVENTOS" in area_resp_upper or "EVENTO" in area_resp_upper
                ):
                    setor_final = "Central de Eventos"
                elif (
                    "FABIANO SOUZA" in area_resp_upper
                    or "LAZER" in area_resp_upper
                    or "CONCIERGE" in area_resp_upper
                ):
                ...
                b_match = next(
                    (benner_index[ek] for ek in keys_emissao if ek in benner_index),
                    None,
                )
                s_match = next(
                    (sabre_index[ek] for ek in keys_emissao if ek in sabre_index), None
                )
                l_match = next(
                    (lemon_index[ek] for ek in keys_emissao if ek in lemon_index), None
                )

                a_vista = clean_num(
                    df_raw.iloc[r, 8] if df_raw.shape[1] > 8 else 0
                )
                a_credito = clean_num(
                    df_raw.iloc[r, 10] if df_raw.shape[1] > 10 else 0
                )
                tarifa_emitida = a_vista + a_credito
                taxa_emitida = clean_num(
                    df_raw.iloc[r, 11] if df_raw.shape[1] > 11 else 0
                )
                comissao = clean_num(
                    df_raw.iloc[r, 13] if df_raw.shape[1] > 13 else 0
                )
                taxa_du = clean_num(
                    df_raw.iloc[r, 14] if df_raw.shape[1] > 14 else 0
                )
                desc = clean_num(df_raw.iloc[r, 15] if df_raw.shape[1] > 15 else 0)
                incentivo = clean_num(
                    df_raw.iloc[r, 18] if df_raw.shape[1] > 18 else 0
                )
                receita_emitida = comissao + taxa_du + incentivo
                vl_liquido = clean_num(
                    df_raw.iloc[r, 20] if df_raw.shape[1] > 20 else 0
                )
                ...
                if b_match:
                    ...
                    status_cia = (
                        "OK"
                        if (
                            nome_esperado_cia in fornec_sistema.upper()
                            or sigla_cia in fornec_sistema.upper()
                        )
                    ...
                    if nome_fonte == "HOT":
                        ...
                        if divs:
                            if len(divs) == 1 and divs[0] == "Cia Aérea":
                                status_divergencia = "Divergência de Cia Aérea"
                            else:
                    ...
                    status_cia = "Pendente"
                    dif_tarifa, dif_taxa, dif_receita = (
                        tarifa_emitida,
                        taxa_emitida,
                        receita_emitida,
                    )
                ...
                # Preserva o Status_Geral definido manualmente pela operação
                if (
                    bilhete_chave in dict_historico
                    and dict_historico[bilhete_chave].get("Status_Geral")
                ):
                    status_geral = dict_historico[bilhete_chave]["Status_Geral"]
                    sg_str = str(status_geral).lower()
                    # Apenas se for registro NÃO CONSTA e a operação sinalizou que já lançou manualmente no ERP
                    if not b_match and any(term in sg_str for term in ["já lançado", "conciliado", "regularizado"]):
                        status_divergencia = "Valores Corretos"
                        aba_destino = "98_OK_Sem_Divergencia_Concil"
                ...
                rec = {
                    ...
                    "Consultor": (
                        s_match["Sabre_AGT"]
                        if s_match
                        else (l_match["Lemon_Consultor"] if l_match else "-")
                    ),
                    ...
                    "Emissor": (
                        emissor_sistema
                        if b_match
                        else (l_match["Lemon_Emissor_Reserva"] if l_match else "-")
                    ),
                    "Sistema Reserva": (
                        sist_reserva
                        if b_match
                        else (l_match["Lemon_Source"] if l_match else "-")
                    ),
                    ...
                    "Emissor_Reserva_Lemon": (
                        l_match["Lemon_Emissor_Reserva"] if l_match else "-"
                    ),
                    "Centro_Custo_Lemon": (
                        l_match["Lemon_Centro_Custo"] if l_match else "-"
                    ),
                    "Forma_Pagto_Lemon": (
                        l_match["Lemon_Forma_Pagto"] if l_match else "-"
                    ),
                    "Autorizacao_Cartao_Lemon": (
                        l_match["Lemon_Autorizacao_Cartao"] if l_match else "-"
                    ),
                }
    ...
    pareto_df = (
        ...
        .sort_values(
            by=["Qtd_Bilhetes", "Tarifa_Pendente_R$"], ascending=False
        )
    )
    ...
    with pd.ExcelWriter(out_file_model, engine="openpyxl") as writer:
        ...
        df_99_suporte.to_excel(
            writer, sheet_name="99_Suporte backoffice", index=False
        )
        df_99_eventos.to_excel(
            writer, sheet_name="99_Central de Eventos", index=False
        )
    ...
    for sheetname in tqdm(
        wb.sheetnames, desc="Formatando Abas do Excel", unit="aba"
    ):
        ws = wb[sheetname]
        ...
        if max_row > 1 and sheetname not in [
            "01_Pareto_Cliente",
            "00_Log_Auditoria",
        ]:
    ...
    print("\n" + "=" * 75)
    print(
        f" AUDITORIA CONCLUÍDA COM SUCESSO! SALVO EM: {out_file_model} ".center(
            75, "="
        )
    )
    print("=" * 75)
